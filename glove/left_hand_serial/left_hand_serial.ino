...
const uint8_t SIG_CANCEL = 0xEE; // Sent when left btn pressed (RECORDING)

// HC12 Calibration update bytes
...
void setup() {
  ...
  Serial.println("--- LEFT HAND (SERIAL MODE) READY ---");
...
// =====================================================
void loop() {
  // --- Receive HC12 commands from Master (Right Hand) ---
  if (HC12.available()) {
    ...
    if (isBtnHeld) {
      if (!actionTriggered && (millis() - btnPressStart > 50)) {
        if (isRecording) {
          ...
          Serial.println("Local data cleared — CANCEL sent");
        } else {
          // DELETE previous recording (when IDLE)
          HC12.write(SIG_DELETE);
          Serial.println("Sent DELETE signal to master");
        }
      }
      isBtnHeld = false;
    }
