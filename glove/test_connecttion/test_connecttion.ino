...
//  
//  String bleServiceName = "PROV_" + mac;
//
//  Serial.println("=================================");
//  Serial.println("DEVICE ID: " + DEVICE_ID);
//  Serial.println("BLE NAME:  " + bleServiceName); // โชว์ชื่อบลูทูธใน Log ด้วย
//  Serial.println("=================================");
//
//  
//
...
void setup() {
  ...
  // 1. สร้าง DEVICE_ID จาก MAC Address (เอาไว้ส่งให้ Flutter)
  String mac = WiFi.macAddress();
  mac.replace(":", "");
  DEVICE_ID = "GLOVE_" + mac;
  ...
  // ==========================================
  // 🌟 3. เริ่มต้นระบบ WiFi & SmartConfig 🌟
  // ==========================================
  WiFi.mode(WIFI_STA);
  WiFi.begin(); // ลองต่อ WiFi เดิมที่เคยจำไว้ในแฟลชก่อน

  Serial.print("Connecting to saved WiFi");
  int waitCount = 0;
  
  // ให้เวลามันลองต่อเน็ตเดิม 5 วินาที (10 รอบ x 500ms)
  ...
  // ถ้าครบ 5 วิแล้วยังไม่ติด -> เข้าโหมดดักฟัง SmartConfig
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("\nNo saved WiFi found. Starting SmartConfig...");
    WiFi.beginSmartConfig();

    // ล็อกการทำงานไว้จนกว่าแอป Flutter จะยิงคลื่น SmartConfig มาสำเร็จ
    while (!WiFi.smartConfigDone()) {
      delay(500);
      Serial.print("*");
      // กระพริบไฟสีน้ำเงินรัวๆ เพื่อเตือนผู้ใช้ว่า "รอรหัส WiFi อยู่นะ"
      setLED(0, 0, 1); 
      delay(100); 
      setLED(0, 0, 0);
    }
    ...
    // ได้รหัสมาแล้ว รอจนกว่าจะเกาะเร้าเตอร์สำเร็จจริงๆ
  ...
  is_connected = true;
  setLED(0, 0, 1); // ไฟสีน้ำเงินติดค้าง = พร้อมใช้งาน
  // ==========================================


  // 4. ตั้งค่าเซนเซอร์และโมดูลต่างๆ (เหมือนเดิมเป๊ะ)
  HC12.begin(115200, SERIAL_8N1, HC12_RX, HC12_TX);
  analogReadResolution(12);
  ...
  // รอจนกว่าจะปล่อยปุ่ม (ป้องกันการกดค้างตอนเปิดเครื่อง)
  while (digitalRead(PIN_BTN_R) == HIGH)
    delay(10);
