...
void loop() {
  ...
  const long interval = 5000; // รอบการส่งข้อมูล 5 วินาที

  // เปลี่ยนไฟเป็นสีเขียวเมื่อต่อ WiFi ติดครั้งแรก
  ...
  // ตรวจสอบว่าเวลาผ่านไปครบ 5 วินาทีหรือยัง
  if (currentMillis - previousMillis >= interval) {
    previousMillis = currentMillis; // อัปเดตเวลาล่าสุด

    // ส่งข้อมูลก็ต่อเมื่อเชื่อมต่อ WiFi สำเร็จแล้ว
    if (is_connected) {
      setLEDColor(LOW, LOW, HIGH); // ไฟสีน้ำเงินกระพริบสั้นๆ ตอนกำลังส่ง
      
      GloveData d;
      readSensors(d);
      sendData(d);
      
      setLEDColor(LOW, HIGH, LOW); // กลับไปเป็นสีเขียวตามปกติ
    }
  }
}
